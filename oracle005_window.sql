...
/*--------------------------------------------------------------------------------
그룹내 순위관련함수
RANK( ) OVER(  ) : 특정 컬럽에 대한 순위를 구하는 함수로 동일한 값에 대해서는 동일한 순위를 준다 .
...
--그룹별로 순위를 부여할 떄 사용 : PARTITION BY
...
SELECT job_id, first_name, salary,row_number() dense_rank() over(ORDER BY salary DESC)
FROM employees;
...
-- CONNECT_BY_ISLEAT : 제일 하위이면 1, 아니면 0으로 리턴한다.
SELECT employee_id, manager_id, LEVEL, lpad(' ', 3*(LEVEL-1)) || first_name,
