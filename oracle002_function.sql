...
--주어진 문자열에서 오른쪾으로 특정문자를 채우는 함수이다.
SELECT rpad('korea', 8, '*')
FROM dual;
...
SELECT to_char(sysdate, 'yyy-mm-dd hh:mi:ss dy') /* 12시간, day:월 */
FROM dual;

SELECT to_char(sysdate, 'yyy-mm-dd hh24:mi:ss dy') /*  24시간, day:월 */
FROM dual;
...
-------------------------------*/
-- 문자 -> 날짜
SELECT to_date('2023_01_04')
FROM dual;

SELECT first_name, hire_date
FROM employees;

--hitr_date(입사일)을 이용해서 '홍길동님은 2023년 1월 3일에 입사했습니다.'로 출력하는 query을 작성하시오.
...
/*----------------------------------------------------------
일반함수(NULL)
1. nvl(컬럼, 대체값) : 첫번쨰 인자값이 null이면 0으로 대체해서 출력한다.
...
/*--------------------------------------------------------------------------
일반함수(NULL)
2.nvl(컬럼, 대체값1, 대체값2) : 컬럼의 값이 null이아니면 대체값1, null이면 대체값2로 출력한다.
----------------------------------------------------------------------------*/
...
sum([DISTINCT] | [ALL] 컬럼): 함계
avg([DISTINCT] | [ALL] 컬럼) : 평균
stddev([DISTINCT] | [ALL] 컬럼) :표준편차
variance([DISTINCT] | [ALL] 컬럼) : 분산
------------------------------------------------------*/
SELECT max(salary), min(salary), sum(salary), avg(salary), stddev(salary), variance(salary)
FROM employees;

SELECT max(DISTINCT salary), min(DISTINCT salary), sum(DISTINCT salary), avg(DISTINCT salary), stddev(DISTINCT salary), variance(DISTINCT salary)
FROM employees;
...
SELECT count(employee_id) --NULL값이 허용안됨
FROM employees; --107
...
FROM employees
GROUP BY department_id; --그룹을 지을때 (count는 그룹(부서)별로 결과가나옴
...
--  50이상이거나 NULL인 부서번호에 대해서 부서별의 직원수를 출력하시오.
SELECT department_id, count(*)
FROM employees
/*WHERE department_id IS NOT NULL*/
GROUP BY department_id 
...
-- 부서별 최소급여, 최대급여를 출력하응게 최소급여와 최대급여가 같지 않은 경우에만 부서별 오름차순으로 출력하시오.
...
--3) 부서별로 커미션을 받는 사원의 수를 구하시오.
  ...
  FROM employees
  --WHERE commission_pct IS NOT NULL 모든 부서별로 볼것인지,받지않는 부서는 제외 후 결과를 볼것인지
  GROUP BY department_id;
...
--4) 부서별 최대급여가 10000이상인 부서만 출력하시오.   
 ...
 FROM employees
 GROUP BY department_id
 HAVING max(salary) >= 10000 --HAVING절은 그룹화에 조건을 달하주는 절
 ORDER BY department_id;
...
--   <출력:  2001		   2002		       2003
 --  	     1          7                6   >
   	 SELECT sum(decode(to_char(hire_date, 'yyy'), '2001', 1, 0)) AS "2001",
          sum(decode(to_char(hire_date, 'yyy'), '2001', 1, 0)) AS "2002",
          sum(decode(to_char(hire_date, 'yyy'), '2001', 1, 0)) AS "2003"
        FROM employees;	   
