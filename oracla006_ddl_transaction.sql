...
SELECT * FROM emp01;
/*
INSERT INTO table_name(column_namw1, column_name2)
VAELUES(*valuel*, values2);
*/
...
도구>환경설정>데이터베이스>객체뷰어>행변경시 사후 편집, 자동 컴밋 설정 체크 해제
*/
...
SELECT dept_id, LEVEL, lpad(' ' , 3*(LEVEL-l)) || dept_name,
       CONNECT_BY_ISCYCLE, parent_id
FROM dept
START WITH dept_id=10
CONNECT BY NOCYCLE dept_id=parent_id;
...
FROM dept
START WITH dept_id=10
CONNECT BY NOCYCLE PRIORdept_id=parent_id;
